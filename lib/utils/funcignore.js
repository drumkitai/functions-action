...
class FuncIgnore {
    ...
    static readFuncignore(working_dir) {
        const funcignorePath = (0, path_1.resolve)(working_dir, ".funcignore");
        const rules = (0, fs_1.readFileSync)(funcignorePath)
            .toString()
            .split("\n")
            .filter((l) => l.trim() !== "");
        try {
    ...
    static removeFilesFromFuncIgnore(working_dir, ignoreParser) {
        ...
        allFiles.forEach((name) => {
            ...
            if (ignoreParser.ignores(filename) &&
                ...
                try {
                    logger_1.Logger.Info(`Removing ${filename}.`);
                    rimraf.sync(name);
                }
                catch (error) {
    ...
    static shouldIgnore(filename) {
        // This is where our http trigger lives.
        const pathsToIgnore = ["v1", "function.json"];
        return pathsToIgnore.some((ignorePath) => filename === ignorePath ||
