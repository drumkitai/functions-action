...
class Kudu {
    ...
    oneDeployFlex(webPackage, queryParameters) {
        return __awaiter(this, void 0, void 0, function* () {
            let httpRequest = {
                method: 'POST',
            ...
            try {
                let response = yield this._client.beginRequest(httpRequest, null, 'application/zip');
                ...
                if (response.statusCode == 200) {
                    ...
                    let deploymentId = response.body;
                    if (!!deploymentId) {
                        core.debug(`Polling for deployment ID: ${deploymentId}`);
                        return yield this._getDeploymentDetailsFromDeploymentID(deploymentId);
                    }
                    else {
    ...
    _getDeploymentDetailsFromDeploymentID(deploymentId) {
        return __awaiter(this, void 0, void 0, function* () {
            let httpRequest = {
                method: 'GET',
                uri: this._client.getRequestUri(`/api/deployments/${deploymentId}`),
            ...
            while (true) {
                ...
                if (response.statusCode == 200 || response.statusCode == 202) {
                    var result = response.body;
                    core.debug(`POLL URL RESULT: ${JSON.stringify(response)}`);
                    if (result.status == exports.KUDU_DEPLOYMENT_CONSTANTS.SUCCESS || result.status == exports.KUDU_DEPLOYMENT_CONSTANTS.PARTIALSUCCESS
                        || result.status == exports.KUDU_DEPLOYMENT_CONSTANTS.CANCELLED || result.status == exports.KUDU_DEPLOYMENT_CONSTANTS.CONFLICT
                        || result.status == exports.KUDU_DEPLOYMENT_CONSTANTS.FAILED) {
                        return result;
                    }
                    else {
                        core.debug(`Deployment status: ${result.status} '${result.status_text}'. retry after 5 seconds`);
